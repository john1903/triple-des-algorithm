...
public class Permutations {
    ...
    public static BitSet[] initialPermutation(BitSet input) {
        ...
        for (int i = 0; i < 64; i++) {
            bitSet.set(i, input.get(initialPermutationTable[i] - 1));
        }
        BitSet left = bitSet.get(0, 32);
        BitSet right = bitSet.get(32, 64);
    ...
    public static BitSet finalPermutation(BitSet left, BitSet right) {
        ...
        for (int i = 0; i < 64; i++) {
            output.set(i, combined.get(finalPermutationTable[i] - 1));
        }
        return output;
    }
    ...
    public static BitSet[] permutedChoice1(BitSet key) {
        ...
        for (int i = 0; i < 56; i++) {
            permutedBits.set(i, key.get(permutedChoice1Table[i] - 1));
        }
        BitSet left = permutedBits.get(0, 28);
        BitSet right = permutedBits.get(28, 56);
        return new BitSet[]{left, right};
    }
    ...
    public static BitSet permutedChoice2(BitSet key) {
        ...
        for (int i = 0; i < 48; i++) {
            permutedKey.set(i, key.get(permutedChoice2Table[i] - 1));
        }
        return permutedKey;
    }
    ...
    public static BitSet extensionPermutation(BitSet input) {
        ...
        for (int i = 0; i < 48; i++) {
            expandedTable.set(i, input.get(extensionTable[i] - 1));
        }
    ...
    public static BitSet pBoxPermutation(BitSet input) {
        BitSet output = new BitSet(32);
        for (int i = 0; i < 32; i++) {
            output.set(i, input.get(pTable[i] - 1));
        }
        return output;
    }
}